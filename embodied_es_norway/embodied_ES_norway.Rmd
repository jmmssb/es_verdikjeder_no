...
Magnus Merkle (SSB), Marte Lindseth (SSB) and Kristine Grimsrud (SSB)
...
-   Added seperate ES accounts, weighted by employment
...
```{r, results = 'hide', message = FALSE, warning=FALSE}
# import data
esdv <- read_excel(file.path(efp,"ES by Industry in Norway.xlsx"), 
                 sheet = "ES by A64 and value added_adj")[-c(1,2),-1] %>% 
  rename(es_sector = `Reference year (2021)`) %>%
    filter(`Ecosystem services` !="Total value added") %>%
...
# same for the employment-weighted version
# import data
esde <- read_excel(file.path(efp,"ES by Industry in Norway.xlsx"), 
                 sheet = "ES by A64 and employment_adj")[-c(1,2),-1] %>% 
  rename(es_sector = `Reference year (2021)`) %>%
    filter(`Ecosystem services` !="Employment by A64") %>%
  select(-c("Ecosystem services")) %>%
  mutate(`Crop provision` = as.numeric(`Crop provision`),
         `Pollination` = as.numeric(`Pollination`),
         `Wood provision` = as.numeric(`Wood provision`),
         `Air filtration` = as.numeric(`Air filtration`),
...
for(i in 1:7){
  # get ES account
  esav<-(esdv[,(i+2)]/xr) %>% as.matrix() %>% as.numeric() %>% matrix(nrow=1)
  esae<-(esde[,(i+2)]/xr) %>% as.matrix() %>% as.numeric() %>% matrix(nrow=1)
  ld[[i]]<- as.data.frame(matrix(ncol=9,nrow=length(sector_codes_c)))
  colnames(ld[[i]])<-c("ES","sector_c","sector_n","embodied_finaldem_v","embodied_exports_v","embodied_gcf_v",
                       "embodied_finaldem_e","embodied_exports_e","embodied_gcf_e")
  ld[[i]]$ES <- colnames(esdv)[i+2]
  ld[[i]]$sector_c <- sector_codes_c
  ld[[i]]$sector_n <- sector_names_c
  ld[[i]]$embodied_finaldem_v <- as.vector(esav %*% leo %*% diag(as.vector(fdr)))
  ld[[i]]$embodied_exports_v <- as.vector(esav %*% leo %*% diag(as.vector(fer)))
  ld[[i]]$embodied_gcf_v <- as.vector(esav %*% leo %*% diag(as.vector(fcr)))
    ld[[i]]$embodied_finaldem_e <- as.vector(esae %*% leo %*% diag(as.vector(fdr)))
  ld[[i]]$embodied_exports_e <- as.vector(esae %*% leo %*% diag(as.vector(fer)))
  ld[[i]]$embodied_gcf_e <- as.vector(esae %*% leo %*% diag(as.vector(fcr)))
}
...
rdfp <- rdf %>% 
  ...
  mutate(agg_groups=factor(agg_groups,levels=c("Agri., forestry, fish.",
                                               ...
                                               "Other services",
                                               "Public services"))) %>%
  mutate(es_weight = "value_added") %>%
  mutate(es_weight = replace(es_weight,item %in% c("embodied_exports_e","embodied_finaldem_e","embodied_gcf_e"),"employment")) %>%
  mutate(item=recode(item,"embodied_exports_v" = "Exports",
                     "embodied_finaldem_v" = "Domestic Final Demand",
                     "embodied_gcf_v" = "Domestic Net Capital Formation",
                     "embodied_exports_e" = "Exports",
                     "embodied_finaldem_e" = "Domestic Final Demand",
                     "embodied_gcf_e" = "Domestic Net Capital Formation")) %>%
...
```{r, results = 'hide', message = FALSE, warning=FALSE}
es_emb_plot<-function(ecos,unites,w="value_added"){
  ggplot(rdfp %>% filter(es_weight == w) %>% 
           filter(ES==ecos),aes(y=value,x=agg_groups,fill=agg_groups)) +
...
Version based on value_added ES weight.
```{r,message = FALSE, results = 'hide'}
es_emb_plot(ecos="Crop provision",unites="k tonnes",w="value_added")
```

Version based on employment ES weight.
```{r,message = FALSE, results = 'hide'}
es_emb_plot(ecos="Crop provision",unites="k tonnes",w="employment")
```
...
Version based on value_added ES weight.
```{r,message = FALSE, results = 'hide'}
es_emb_plot(ecos="Pollination",unites="k tonnes",w="value_added")
```

Version based on employment ES weight.
```{r,message = FALSE, results = 'hide'}
es_emb_plot(ecos="Pollination",unites="k tonnes",w="employment")
```
...
Version based on value_added ES weight.
```{r,message = FALSE, results = 'hide'}
es_emb_plot(ecos="Wood provision",unites="k cubic metres",w="value_added")
```

Version based on employment ES weight.
```{r,message = FALSE, results = 'hide'}
es_emb_plot(ecos="Wood provision",unites="k cubic metres",w="employment")
```

# 4.5 Air filtration

Version based on value added ES weight.
```{r,message = FALSE, results = 'hide'}
es_emb_plot(ecos="Air filtration",unites="Tonnes PM2.5",w="value_added")
```

Version based on employment ES weight.
```{r,message = FALSE, results = 'hide'}
es_emb_plot(ecos="Air filtration",unites="Tonnes PM2.5",w="employment")
```
...
Version based on value added ES weight.
```{r,message = FALSE, results = 'hide'}
es_emb_plot(ecos="Flood control",unites="hectares",w="value_added")
```

Version based on employment ES weight.
```{r,message = FALSE, results = 'hide'}
es_emb_plot(ecos="Flood control",unites="hectares",w="employment")
```
...
Version based on value added ES weight.
```{r,message = FALSE, results = 'hide'}
es_emb_plot(ecos="Local climate regulation",unites="degrees",w="value_added")
```

Version based on employment ES weight.
```{r,message = FALSE, results = 'hide'}
es_emb_plot(ecos="Local climate regulation",unites="degrees",w="employment")
```
...
Version based on value added ES weight.
```{r,message = FALSE, results = 'hide'}
es_emb_plot(ecos="Nature based tourism",unites="overnight stays",w="value_added")
```

Version based on employment ES weight.
```{r,message = FALSE, results = 'hide'}
es_emb_plot(ecos="Nature based tourism",unites="overnight stays",w="employment")
```
